...
",
    ...
    // The table names some functions more than once — `ai_place` twice,
    // `vs_cast` seven times — as overloads by argument count. This module is
    // keyed by name, so each name gets one entry: its first slot, how many
    // overloads there are, and a stub mark only if every overload is one.
    let mut first_by_name: BTreeMap<&str, &FunctionEntry> = BTreeMap::new();
    let mut in_order: Vec<&FunctionEntry> = Vec::new();
    for f in functions {
        if !first_by_name.contains_key(f.name.as_str()) {
            first_by_name.insert(&f.name, f);
            in_order.push(f);
        }
    }
    for first in in_order {
        let overloads: Vec<&FunctionEntry> =
            functions.iter().filter(|f| f.name == first.name).collect();
        let all_stubs = overloads.iter().all(|f| f.evaluate_rva == stub);
        ...
        let text = first
            ...
            .unwrap_or_default();
        let overload_note = if overloads.len() > 1 {
            format!(", overloads = {}", overloads.len())
        } else {
            String::new()
        };
        let stub_note = if all_stubs { ", stub = true" } else { "" };
        s.push_str(&format!(
            "    [{}] = {{ index = {}, returns = {}, flags = {}, params = {{ {} }}{text}{overload_note}{stub_note} }},
",
            lua_string(&first.name),
            first.index,
            lua_string(&types[first.returns as usize]),
            first.flags,
            params.join(", "),
        ));
    }
...
",
    ...
    let mut seen_globals: BTreeMap<&str, ()> = BTreeMap::new();
    for g in globals {
        if seen_globals.insert(&g.name, ()).is_some() {
            continue;
        }
        let dead_note = if g.storage_rva == 0 {
            ", dead = true"
        } else {
            ""
        };
        s.push_str(&format!(
            "    [{}] = {{ index = {}, type = {}{dead_note} }},
",
            lua_string(&g.name),
            g.index,
